...
namespace Sonar
{
    ...
    void GameState::Check3PiecesForMatch( int x1, int y1, int x2, int y2, int x3, int y3, int pieceToCheck )
    {
        ...
        {
            std::string winningPiecesStr;
            
            if ( O_PIECE == pieceToCheck )
            {
                winningPiecesStr = "O Winning Piece";
            }
            else
            {
                winningPiecesStr = "X Winning Piece";
            }
            
            _gridPieces[x1][y1].setTexture( this->_data->assets.GetTexture( winningPiecesStr ) );
            _gridPieces[x2][y2].setTexture( this->_data->assets.GetTexture( winningPiecesStr ) );
            _gridPieces[x3][y3].setTexture( this->_data->assets.GetTexture( winningPiecesStr ) );
